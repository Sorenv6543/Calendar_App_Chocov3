...
const refreshTurnDayHighlights = (): void => {
  ...
  requestAnimationFrame(() => {
    ...
    const turnDates = events
      .filter(
        (event) => event.extendedProps?.turn === true && event.extendedProps?.turndate
      )
...
const calendarOptions: CalendarOptions = {
  ...
  eventDidMount: (info: {
    el: HTMLElement;
    event: { extendedProps: { color?: string } };
  }) => {
    info.el.style.setProperty(
      "--event-color",
      info.event.extendedProps.color || "#2979ff"
    );
  ...
  dayCellDidMount: (arg: { date: Date; el: HTMLElement }) => {
    ...
    const hasTurnOnDay = events.some((event) => {
      return (
        event.extendedProps?.turn === true && event.extendedProps?.turndate === cellDate
      );
    });
...
watch(
  ...
  (newHouseId) => {
    if (newHouseId && userStore.userData?.houses) {
      const house = userStore.userData.houses.find(
        (h: { houseId: string }) => h.houseId === newHouseId
      );
      if (house) {
...
<template>
  <div class="calendar-container">
    ...
    <FullCalendar
      class="full-calendar v-elevation-25"
      ref="calendarRef"
